...
from loss import calc_gan_loss, words_loss, sent_loss, InpaintLoss
...
parser.add_argument('--root', type=str, default='F:/FCJ/102flower/')
...
parser.add_argument('--save_dir', type=str, default='../model/flower')
parser.add_argument('--training_image', type=str, default='./training_flower')
...
if __name__ == "__main__":
    ...
    inpaint_loss = InpaintLoss().to(device)
    ...
    if args.resume:
        ...
        pd_checkpoint = torch.load(f'{args.save_dir}/D_{args.resume}.pth', map_location=device)
    ...
    for i in range(start_epoch, args.max_epoch):
        ...
        while train_step < num_batches:
            ...
            refine_result_t = g_model(masked, mask, noise, sent_emb, words_embs, text_mask)
            ...
            pg_loss_t, pd_loss_t = calc_gan_loss(pd_model, refine_result_t, img, words_embs)
            refine_inpaint_loss = inpaint_loss(refine_result_t, img, mask)
            recon_loss_t = 1.0 * l1(refine_result_t, img)

            gan_loss_t = pg_loss_t
            total_loss_t = 1.0 * recon_loss_t + 0.002 * gan_loss_t + 0.001 * matching_loss + refine_inpaint_loss
            ...
            if num_save_interval % args.save_model_interval == 0 or (i + 1) == args.max_epoch:
                ...
                torch.save(pd_model.state_dict(), f'{args.save_dir}/D_{num_save_interval}.pth')
                print("model saved.")
                if (i+1) == args.max_epoch:
                    break
            ...
            if num_save_interval % args.log_interval == 0:
                ...
                print('g_loss_t/inpaint_loss_t', refine_inpaint_loss.item())
                print('g_loss_t/gan_loss', gan_loss_t.item())
